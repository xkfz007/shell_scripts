...
do
    ...
    do
        ...
        tbits=`awk -v x=$tbr -v y=$fps 'BEGIN{printf "%f",x*1000/y}'`

        col=`grep 'POC    0' $cons |awk '{for(i=1;i<=NF;i++) if( $i =="fsz=" ){print i;break} }'`
        ((col++))
#        qp1=`grep "POC    0" $cons|awk -v x=$col '{print $x}'`
        ...
        do
            ...
            ((line_no+=1))
            qp2=`awk -v x=$qp_tmp -v y=$qp2 'BEGIN{printf "%f",x+y}'`
            printf "."
        done
        printf "\n"
        qp2=`awk -v x=$qp2 -v y=$frames 'BEGIN{printf "%f",x/y}'`
        echo $i $tbits $qp2 >>$tt
    done
