...
do
    ...
    do
        ...
        line_no2=`grep -n "SUMMARY" $cons |awk -F: '{print $1}'` 
        ((line_no2+=2))
        frames=`sed -n -e "${line_no2}"'p' $cons |awk '{print $1}'`

        ((line_no+=frames-1))

        col=`grep 'POC    0' $cons |awk '{for(i=1;i<=NF;i++) if( $i =="clip="){print i;break} }'`
        ((col++))

        clip=`sed -n -e "${line_no}"'p' $cons |awk -v x=$col '{print $x}'`

        echo "${cons%.txt} $clip" >>$tt
    done
